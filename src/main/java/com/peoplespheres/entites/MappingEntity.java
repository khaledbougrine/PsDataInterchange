...
public class MappingEntity extends AAuditableVersionedEntity {
    ...
    public MappingEntity(@NonNull final MappingEntity referenceMappingEntity) {
        ...
        this.setTransformations(referenceMappingEntity.getTransformations().stream().filter(Objects::nonNull).map(transform -> new TransformationEntity(transform)).collect(Collectors.toSet()));
    }
}
