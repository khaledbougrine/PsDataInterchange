...
public class FlowDTO {
    ...
    @Schema(description = "PSO type associated to this flow (ex: usr)", example = "usr", required = true)
    @NotBlank(message="{flow.pso-type.not-blank}")
    private String psoType;

    @Schema(description = "Flow name (label)", example = "User", required = true)
    @NotBlank(message="{flow.name.not-blank}")
    private String name;

    @Schema(description = "Type", allowableValues = {"IN", "OUT"}, required = true)
    @NotNull(message="{flow.type.not-null}")
    private FlowDirectionEnum type;

    @Schema(description = "Flow version (ex:1.01)", example = "1.0", required = true)
    @NotBlank(message="{flow.version.not-blank}")
    private String flowVersion;

    @Schema(description = "Status", allowableValues = {"DRAFT", "RELEASE"}, required = true)
    ...
    @Schema(description = "Execution type", allowableValues = {"REAL_TIME", "SCHEDULED"}, required = true)
    ...
    @Schema(description = "Is this flow currently activated ?", required = true)
    @NotNull(message = "{flow.active.not-null}")
    private Boolean isActive = true;

    @Schema(description = "Has this flow already been visualised ?", required = true)
    ...
    @Schema(description = "Unique identifier of the associated connector", required = true)
    @NotNull(message="{flow.connector.id.not-null}")
    @PositiveOrZero(message="{flow.connector.id.positive}")
    private Long connectorId;

    @Schema(description = "Unique identifier of the targeted partner module", required = true)
