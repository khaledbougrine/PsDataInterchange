...
public class ScheduleDTO implements Serializable {
    @Schema(description = "The unique identifier of the schedule", required = true)
    @NotNull(message="{schedule.id.not-null}")
    @PositiveOrZero(message="{schedule.id.positive}")
    private Long id;

    @Schema(description = "Boolean flag indicating if the scheduled flow associated is in maintenance mode", allowableValues = {"true", "false"}, required = true)
    ...
    @Schema(description = "Unique identifier of the associated client", required = true)
    @NotBlank(message="{schedule.client.id.not-blank}")
    @Size(min = 36,max = 36, message = "{schedule.client.id.uuid-format}")
    private String clientId;

    @Schema(description = "Unique identifier of the associated flow", nullable = true)
    @Nullable
    private Long flowId;
}
