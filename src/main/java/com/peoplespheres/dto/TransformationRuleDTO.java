...
public class TransformationRuleDTO implements Serializable {
    @Schema(description = "TransformationDomain rule technical ID", required = true)
    @NotNull(message="{transformation.rule.id.not-null}")
    @PositiveOrZero(message="{transformation.rule.id.positive}")
    private Integer id;

    @Schema(description = "The displayed name of this transformation rule", required = true)
    ...
    @Schema(description = "The function associated to this transformation rule (ex:text)", example = "text",
            allowableValues = {"alias", "text", "list", "hierarchy", "checkbox", "multi_checkbox", "radio_button",
                    ...
                    "user_relation", "group", "latitude", "longitude", "modules", "domain", "external_id", "string"}, required = true)
    @NotNull(message="{transformation.rule.function.not-null}")
    private RuleTypeEnum function;

    @Schema(description = "The minimum number of parameters required for this specific transformation rule (ex: 0)", example = "0", required = true)
    ...
    @Schema(description = "The maximum number of parameters required for this specific transformation rule (ex: 3)", example = "3", required = true)
    ...
    @Schema(description = "Boolean flag indicating if the transformation rule is purely internal", required = true)
